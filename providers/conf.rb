action :create do
  ...
  if node["cinder"]["storage"]["backends"].any?
    storage_backend_options = {}
    node["cinder"]["storage"]["backends"].each do |backend_key,backend_options|
      storage_backend_options[backend_key] = storage_options.merge(backend_options)
    end
    enabled_backends = storage_backend_options.keys.join(',')
